...
from bonoptions.core import OptionWorld
...
def test_planner_generates_exact_budget() -> None:
    planner = BestOfNPlanner(OptionWorld.default(seed=0), ProxyScorer())
...
def test_proxy_and_true_utility_are_separate_quantities() -> None:
    planner = BestOfNPlanner(OptionWorld.default(seed=0), ProxyScorer(mode="miscalibrated"))
...
def test_boundary_sieve_does_not_reference_hidden_true_labels() -> None:
    source = inspect.getsource(BoundaryCalibratedOptionSieve.public_boundary_score)
    source += inspect.getsource(BoundaryCalibratedOptionSieve.rank_score)
    source += inspect.getsource(BoundaryCalibratedOptionSieve.select)
...
def test_boundary_sieve_can_choose_lower_proxy_safer_plan() -> None:
    ...
    raw = planner.select_raw(candidates)
    repaired = BoundaryCalibratedOptionSieve().select(candidates)
...
def test_finite_n_law_matches_manual_two_item_population() -> None:
    planner = BestOfNPlanner(OptionWorld.default(seed=0), ProxyScorer(mode="miscalibrated"))
    candidates = sorted(planner.generate_candidates(n=2, horizon=2, seed=9), key=lambda p: p.proxy_score)
    expected = finite_n_selected_expectation(candidates, ns=[1, 2], metric="true_utility")
    low, high = candidates
